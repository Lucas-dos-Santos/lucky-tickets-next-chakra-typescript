...
import { HStack, Icon, Flex, Heading } from '@chakra-ui/react';
...
function Countdown({ expiration }: CountdownProps) {
  return expiration ? (
    <CountdownComponent date={new Date(expiration * 1000)} renderer={({ hours, minutes, seconds }) => Render({ hours, minutes, seconds })} />
  ) : (
    <></>
  )
}
